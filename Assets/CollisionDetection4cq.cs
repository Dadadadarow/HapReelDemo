...
public class CollisionDetection4cq : MonoBehaviour
{
    public DataSender dataSender;
    public TaskManager taskMode;
    void OnCollisionEnter(Collision collision)
    {
        ...
        if (taskMode.isTorque)
        {
            ...
            {
                dataSender.SendMotorControlPacket(4, 0.2f, 10, 0.0f, 30f, 10.0f);
                // Debug.Log("Detect 1");
            }
            else if (-0.02f <= localCollisionPoint.x && localCollisionPoint.x < 0.02f)
            {
                dataSender.SendMotorControlPacket(2, 0.2f, 0, 0.0f, 50f, 10.0f);
                // Debug.Log("Detect 2");
            }
            else if (0.02f <= localCollisionPoint.x && localCollisionPoint.x < 0.06f)
            {
                dataSender.SendMotorControlPacket(3, 0.2f, 10, 0.0f, 30f, 10.0f);
                // Debug.Log("Detect 3");
            }
        ...
        else
        {
            ...
            if (-0.1f <= localCollisionPoint.x && localCollisionPoint.x < -0.06)
            {
                dataSender.SendMotorControlPacket(3,0.2f, 0, 0.0f, 10f, 10.0f);
                // Debug.Log("Detect 0");
            }
            else if (-0.06f <= localCollisionPoint.x && localCollisionPoint.x < -0.02f)
            {
